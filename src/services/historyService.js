...
// Normalizes incoming values so matrices and vectors only store non-negative numbers.
function toNumber(value) {
  return Math.max(0, Number(value ?? 0));
}

// Creates a deep numeric copy of a matrix before it is analyzed or persisted.
function cloneMatrix(matrix = []) {
  return matrix.map((row) => row.map((value) => toNumber(value)));
}

// Builds the per-user browser storage key used for local history fallback.
function getStorageKey(userId) {
  return `${STORAGE_PREFIX}${userId}`;
}

// Generates a stable signature for comparing scenarios across local and cloud history.
...
// Runs Banker's Algorithm to capture the analysis that will be saved with a history entry.
...
// Reads the current user's locally cached history from the browser.
...
// Stores the latest merged history list in browser storage for offline fallback.
...
// Converts raw local or Firestore documents into the app's history entry shape.
...
// Deduplicates, sorts, and trims history entries from local and cloud sources.
...
// Builds a complete history snapshot from the current simulator state.
  ...
  return {
    ...
    snapshot: {
      ...
      analysis: {
        ...
        safeSequence: analysis.safeSequence,
        blockedProcesses: analysis.blockedProcesses,
        blockedProcessDetails: analysis.blockedProcessDetails,
        waitCycle: analysis.waitCycle,
...
// Loads a user's history from Firestore and falls back to browser storage if needed.
...
// Saves a new history entry locally first, then attempts to sync it to Firestore.
