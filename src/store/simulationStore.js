...
function buildClearedAnalysisState() {
  return {
    isSafe: null,
    blockedProcessIds: [],
    safeSequence: null,
    ...
    recoveryMode: null,
    messageLog: [],
    analysisNarration: null,
    lastRecoveryAction: null,
  };
}

function describeReleasedResources(resources, released) {
  const releasedItems = released
    .map((value, resourceIndex) => ({
      label: resources[resourceIndex]?.id || `R${resourceIndex}`,
      value: Number(value ?? 0),
    }))
    .filter((entry) => entry.value > 0)
    .map((entry) => `${entry.label} x${entry.value}`);

  return releasedItems.length > 0 ? releasedItems.join(', ') : 'no held resources';
}

function buildSafetyMessage(result) {
  if (result.isSafe) {
    return `Safe state confirmed. ${result.narration?.whySafeSequence || `Sequence: ${result.safeSequence.join(' -> ')}.`}`;
  }

  return `Unsafe state detected. ${result.narration?.deadlockReason || result.narration?.safeSequenceFailure || `Blocked processes: ${result.blockedProcesses.join(', ')}.`}`;
}

function buildRecoveryActionSummary({
  mode,
  processId,
  priority = null,
  released,
  ...
  const releasedText = describeReleasedResources(resources, released);
  const explanation =
    mode === 'terminate'
      ? `${processId} was terminated to release ${releasedText}.`
      : `${processId} had the lowest priority${priority !== null ? ` (${priority})` : ''}, so it was preempted and released ${releasedText}.`;
  ...
  return {
    ...
    title:
      mode === 'terminate'
        ? `Termination applied to ${processId}`
        : `Preemption applied to ${processId}`,
    explanation,
    outcome: result.isSafe ? 'safe' : 'unsafe',
    outcomeSummary: result.isSafe
      ? result.narration?.whySafeSequence || 'The released resources restore a safe execution order.'
      : result.narration?.safeSequenceFailure ||
        result.narration?.deadlockReason ||
        'The system remains blocked after this recovery step.',
  };
}
...
export const useSimulationStore = create((set, get) => ({
  ...
  runSafetyCheck: () =>
    set((state) => {
      ...
      return {
        ...
        analysisNarration: result.narration,
        lastRecoveryAction: null,
  ...
  terminateProcess: (processId) =>
    set((state) => {
      ...
      const recoveryAction = buildRecoveryActionSummary({
        mode: 'terminate',
        processId,
        released,
        resources: recalculated.resources,
